...
def generate_multilingual_analysis(product_name, data, language):
    ...
    AND includes the raw scores for the app to use.
    """
    ...
    # --- Build the text-based analysis ---
    text_response = {}
    if data["ml_label"] == 'ok':
        text_response['title'] = lang["good_title"]
        text_response['summary'] = lang["good_summary"]
    else:
        text_response['title'] = lang["mindful_title"]
        text_response['summary'] = lang["mindful_summary"]
    ...
    text_response['scoreInsights'] = score_insights

    if gi > 70 and nutri_score > 10:
        text_response['proTip'] = lang["pro_tip_bad"]
    else:
        text_response['proTip'] = lang["pro_tip_good"]
    
    # --- Combine text and scores into the final response ---
    final_response = {
        "analysis": text_response,
        "scores": {
            "nutriScore": nutri_score,
            "ecoScore": round(eco_score),
            "glycemicIndex": round(gi)
        }
    }
...
def analyze_product():
    try:
        ...
        # Generate the multilingual response which now includes scores
